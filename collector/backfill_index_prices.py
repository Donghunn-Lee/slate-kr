"""
KRX Marketplace 지수 일봉 → index_daily_prices 대량 백필 (idempotent upsert).

용도
  대량 과거 구간 백필 전용. 일일 갱신은 fetch_index_prices.py (KIS) 소관.
  KRX Marketplace 는 익일 08:00 KST 데이터 갱신 특성상 당일 EOD 회귀에
  부적합하나, 과거 구간 정합·재적재는 신뢰도가 높아 백필 소스로 유지.

사용
  python backfill_index_prices.py --backfill YYYY-MM-DD YYYY-MM-DD
    · 주말은 사전 스킵, 공휴일은 KRX 빈 응답으로 skip
    · 지수 4종(KOSPI/KOSDAQ/KOSPI200/KOSDAQ150) 동시 처리

호출 (영업일당 2회)
  idx/kospi_dd_trd   → IDX_NM '코스피' + '코스피 200' 두 건 추출
  idx/kosdaq_dd_trd  → IDX_NM '코스닥' + '코스닥 150' 두 건 추출

규약: psycopg2 / load_dotenv / logs/{prefix}_{YYYYMMDD}.log /
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="KRX 지수 일봉 대량 백필 → index_daily_prices upsert. "
                    "일일 갱신은 fetch_index_prices.py (KIS) 소관."
    )
    parser.add_argument(
        "--backfill",
        nargs=2,
