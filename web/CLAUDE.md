...
- 작업 맥락을 코드에 남기지 않는다: 세션·프롬프트 번호, 이슈 ID(E69 등), "이번 작업", "지시에 따라", "기존 동작 유지" — 커밋 메시지·개발 일지 소관
- 변경 이력을 서술하지 않는다: "~에서 ~로 변경", "이전에는" — git 소관
- 시점 종속 표현을 피한다: "현재는", "아직", "임시로". 남겨야 하면 `TODO(사유)` 형태
- 쓸 가치가 있는 주석: 비자명한 외부 제약(unstable_cache는 null도 캐시, Neon bigint는 string 등), 직관에 반하는 선택의 이유, 도달 불가로 보이지만 남긴 방어 분기의 근거
- 언어·톤은 해당 파일의 기존 주석을 따른다
- 판별 기준: **이 파일이 처음부터 이렇게 쓰였다고 가정해도 성립하는 문장인가.** 성립하면 남긴다. "~하지 않는 이유"(대안 배제 근거)는 성립하므로 허용, "~였을 때"·"이전 구현은"은 성립하지 않으므로 금지
- 테스트 주석은 케이스 선정 근거만 쓴다(왜 이 경계값인가). 케이스가 무엇을 검증하는지는 테스트 이름 소관
- 주석을 지웠을 때 독자가 되묻게 될 질문이 없다면 그 주석은 "무엇"이다 — 삭제
...
- 본문은 명령형·영문 불릿 2~4개, 첫 글자 대문자. "어떻게"가 아니라 "무엇을"
- 동기가 자명하지 않은 커밋(`fix`·`refactor`·되돌림 등)은 불릿 위에 이유(영문) 1~2문장을 쓴다. 자명한 `feat`은 불릿만
